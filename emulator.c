...
void execute_itype_except_load(Instruction instruction, Processor *processor) {
    ...
    switch (instruction.itype.funct3) {
        ...
        case 0x2:
            //slti
            processor->R[instruction.itype.rd] = (sWord)processor->R[instruction.itype.rs1] < imm ? 1 : 0;
            break;
        case 0x4:
            //xori
            processor->R[instruction.itype.rd] =
...
void execute_load(Instruction instruction, Processor *processor, Byte *memory) {
    ...
    switch (instruction.itype.funct3) {
        ...
        //lb
        case 0x0:
            processor->R[instruction.itype.rd] = (sByte)(load(memory, address, LENGTH_BYTE));
            break;
        //lh
        case 0x1:
            processor->R[instruction.itype.rd] = (sHalf)(load(memory, address, LENGTH_HALF_WORD));
            break;
        //lw
        case 0x2:
            processor->R[instruction.itype.rd] = (sWord)load(memory, address, LENGTH_WORD);
            break;
        default:
            handle_invalid_instruction(instruction);
            exit(-1);            
...
void execute_store(Instruction instruction, Processor *processor, Byte *memory) {
    Address address = processor->R[instruction.stype.rs1] + get_store_offset(instruction);
    switch (instruction.stype.funct3) {
        /* YOUR CODE HERE */
        case 0x0: // sb
            store(memory, address, LENGTH_BYTE, processor->R[instruction.stype.rs2] & 0xFF);
            break;
        case 0x1: // sh
            store(memory, address, LENGTH_HALF_WORD, processor->R[instruction.stype.rs2] & 0xFFFF);
            break;
        case 0x2: // sw
